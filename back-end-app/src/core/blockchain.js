...
class Blockchain {
  ...
  generateTransaction(sender, recipient, amount, privateKey) {
    ...
    const txIns = newTransaction.gererateTxIns(
      sender,
      amount,
      uTxOutsByAddress
    );
  ...
  transactionsIsValid(transactions) {
    ...
    transactions.forEach((tx) => {
      ...
      for (const txIn of newTx.txIns) {
        if (txIn.txOutId !== "0") {
          const refUTxOut = this.unspentTxOuts.filter(
  ...
  getTransactionsByAddress(address) {
    ...
    const refUTxOuts = this.unspentTxOuts.filter(
    ...
    if (refUTxOuts.length !== 0) {
      refUTxOuts.forEach((uTxOut) => {
        this.blocks.forEach((element) => {
          element.transactions.forEach((tx) => {
            ...
            tx.txIns.forEach((txIn) => {
              if (
                ...
                newTx = {
                  ...newTx,
                  from: `${
                    txIn.txOutIndex === 0 ? "system send" : "coinbase (reward)"
                  }`,
                };
              }
            ...
            if (uTxOut.txOutId === tx.txOuts[0].txOutId) {
              const sender = this.unspentTxOuts.filter(
                (uTxOut) => uTxOut.txOutId === tx.txIns[0].txOutId
              );
              if (sender.length !== 0) {
        ...
        this.pendingTransactions.forEach((pTransaction) => {
          const txIns = pTransaction.txIns.filter(
            (txIn) => txIn.txOutId === uTxOut.txOutId
          );
          if (txIns.length > 0) {
            let newTx = {
    ...
    this.pendingTransactions.forEach((pTransaction) => {
      if (pTransaction.txOuts[0].address === address) {
        const sysSend =
          pTransaction.txIns[0].txOutId === "0" &&
          pTransaction.txIns[0].txOutIndex === 0
            ? enumSysSender.COIN_SYSTEM
            : "";
        const txInsSend = this.unspentTxOuts.filter(
          (uTxOut) => pTransaction.txIns[0].txOutId === uTxOut.txOutId
        );
        let newTx = {
          hash: pTransaction.hash,
          timeStamp: new Date().toString(),
  ...
  getAllTransactions() {
    ...
    // get unique address from unspentTxOuts
    const allWalletAddress = [
      ...new Set(this.unspentTxOuts.map((uTxOut) => uTxOut.address)),
      ...new Set(this.pendingTransactions.map((tx) => tx.txOuts[0].address)),
    ];

    allWalletAddress.forEach((address) => {
      const transactions = this.getTransactionsByAddress(address); // list of transactions for this address
      allTransaction.push(...transactions);
    });
    return allTransaction;
  }

  getUTxOutsByAddress(address) {
    return this.unspentTxOuts.filter(
      (uTxOut) => uTxOut.address === address && uTxOut.status === 1
    );
  }

  getBalance(address) {
    const uTxOutsOfAddress = this.unspentTxOuts.filter(
      (uTxOut) => uTxOut.address === address && uTxOut.status === 1
    );
...
class Transaction {
  ...
  gererateTxOuts(sender, recipient, txInBalance, amount) {
    ...
    if (systemSender.includes(sender)) {
      ...
      if (refund > 0) {
        const uTxOutrefund = new UnspentTxOut({
          address: sender,
          amount: refund,
        });
...
class TxIn {
  ...
  signTxIn(hashTx, privateKey, unspentTxOuts) {
    ...
    const refUTxOut = unspentTxOuts.filter(
      (uTxOut) => uTxOut.txOutId === this.txOutId
    )[0];
...
class UnspentTxOut {
  ...
  hashUnspentTxOut(hashTx) {
    const stringHash = `${hashTx}${this.TxOutIndex}${this.address}${
      this.amount
    }${Date.now()}`;
