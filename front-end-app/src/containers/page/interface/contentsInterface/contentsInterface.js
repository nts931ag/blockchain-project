                        ...
                        {(() => {
                            switch (optionModal) {
                                case enumOptionModal.WALLET:
                                    return <WalletModal closeModal={() => {setupModal()}}></WalletModal>;
                                case enumOptionModal.LOGOUT:
                                    return <LogoutModal closeModal={() => {setupModal()}}></LogoutModal>;
                                case enumOptionModal.SEND:
                                    return <SendTransactionModal setTransaction={setNewTx} blockchain={blockchain} transaction={newTx} setupModal={setupModal}></SendTransactionModal>;
                                default:
                                    break;
                            }
                        })()}
...

                {
                    (()=>{
                        switch (option) {
                            ...
                            case 2:
                                return <SendTransaction transaction={newTx} setTransaction={setNewTx} walletBalance={balance} openModal={()=> {setupModal(enumState.VISIBLE, enumOptionModal.SEND)}}></SendTransaction>
